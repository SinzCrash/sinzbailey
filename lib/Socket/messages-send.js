...
export const makeMessagesSocket = (config) => {
    ...
    // ============================================================
    // ⭐ FIX 1: FUNGSI getMessageType DENGAN VALIDASI
    // ============================================================
    const getMessageType = (message) => {
        // VALIDASI: cek apakah message valid
    ...
    // ============================================================
    // ⭐ FIX 2: FUNGSI getMediaType DENGAN VALIDASI
    // ============================================================
    ...
    // ============================================================
    // ⭐ FIX 3: FUNGSI getButtonType DENGAN VALIDASI (YANG ERROR)
    // ============================================================
    const getButtonType = (message) => {
        // VALIDASI: ini yang paling penting!
        if (!message || typeof message !== 'object') {
            return undefined;
        }
    ...
    // ============================================================
    // ⭐ FIX 4: FUNGSI relayMessage DENGAN VALIDASI DI AWAL
    // ============================================================
    const relayMessage = async (
        ...
        // ============================================================
        // ⭐ VALIDASI UTAMA: cek message sebelum diproses
        // ============================================================
        if (!message || typeof message !== 'object') {
            logger.error({ jid, messageType: typeof message }, 'relayMessage: message invalid or undefined');
            return null;
        }

        // Cek apakah message punya konten
        const hasContent = Object.keys(message).some(key => 
            message[key] !== undefined && message[key] !== null
        );

        if (!hasContent) {
            logger.error({ jid, keys: Object.keys(message) }, 'relayMessage: message has no content');
            return null;
        }
        ...
        // AMAN: getButtonType sudah ada validasi
    ...
    return {
        ...
        // ============================================================
        // ⭐ FIX 5: sendMessage DENGAN VALIDASI
        // ============================================================
        sendMessage: async (jid, content, options = {}) => {
            // VALIDASI: cek content
            if (!content) {
                logger.error({ jid }, 'sendMessage: content is empty');
                return null;
            }
            ...
            if (typeof content === 'object' &&
                ...
                // VALIDASI: cek fullMsg
                if (!fullMsg || !fullMsg.message) {
                    logger.error({ jid, content }, 'sendMessage: failed to generate message');
                    return null;
                }
        ...
        // ============================================================
        // ⭐ FIX 6: sendMessageMembers DENGAN VALIDASI
        // ============================================================
        sendMessageMembers: async (jid, message, options = {}) => {
            // VALIDASI: cek message
            if (!message || typeof message !== 'object') {
                logger.error({ jid }, 'sendMessageMembers: message invalid');
                return null;
            }
            ...
            for (let i = 0; i < participantJids.length; i++) {
                const targetJid = participantJids[i];
                if (areJidsSameUser(targetJid, meId)) continue;

                try {
                    const msgId = `${idm || Utils_1.generateMessageID()}_${i}`;
                    const fullMsg = await Utils_1.generateWAMessageFromContent(targetJid, message, {
                        messageId: msgId,
                        quoted
                    });
                    ...
                    // VALIDASI: cek fullMsg
                    if (!fullMsg || !fullMsg.message) {
                        logger.error({ targetJid }, 'sendMessageMembers: failed to generate message');
                        continue;
                    }
                    ...
                    logger.debug(`Message successfully sent to ${targetJid}`);

                    if (delayMs && i < participantJids.length - 1) {
                        await new Promise(z => setTimeout(z, delayMs));
                    }
                } catch (e) {
                    logger.error({ targetJid, e }, "Error sending message to");
                }
            }
